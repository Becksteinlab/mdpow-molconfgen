...
from .. import sampler
from .. import chem
from ..data.files import V46_PDB, V46_ITP, V36_ITP, V36_PDB


@pytest.fixture(params=[
    ("V46", V46_ITP, V46_PDB, [[2, 1, 0, 4]]),  # V46 dihedral indices
    ("V36", V36_ITP, V36_PDB, [[7, 10, 0, 1], 
                               [10, 0, 1, 3]]),  # V36 dihedral indices
])
def molecule_data(request):
    """Create test universes for both V46 and V36 molecules."""
...
def test_generate_conformers(molecule_data, num_conformers=5):
    """Test generating conformers for V46 and V36 molecules."""
    name, universe, dihedral_indices = molecule_data
    
    # Create a list of dihedrals using a list comprehension
    dihedrals = [universe.atoms[indices].dihedral for indices in dihedral_indices]
